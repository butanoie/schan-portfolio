...
- **Post-load-more state transitions need extended timeout** — After the final Load More click, `toHaveCount(N)` passes once projects render, but React still needs additional render cycles to flip `hasMore` → `false` and unmount the button. WebKit in CI is slow enough that `toBeHidden()` with the default 5s timeout fails. Use `{ timeout: 10_000 }` for assertions that depend on state transitions after async operations complete.
