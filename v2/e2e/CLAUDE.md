...
- **axe-core + CSS opacity transitions** — axe computes incorrect contrast ratios when scanning during a Fade/opacity transition (element at partial opacity). POM wait methods must include a post-visibility delay (e.g., 500ms) to let transitions settle before running `runAxeScan`.
